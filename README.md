...
### 로또 번호 에러 처리 클래스
- [X] 모두 숫자로 이루어져있지 않은 경우 에러 처리 한다.
- [X] 중복된 번호가 있을 경우 에러 처리 한다.
- [X] 1부터 45까지의 숫자가 아닌 경우 에러 처리 한다.

### 단위 기능 테스트 코드
- 구입 금액
    - [X] 빈값으로 입력한 경우 에러 처리 한다. ('')
    - [X] 0원 입력인 경우 에러 처리 한다. ('0')
    - [X] 1,000원 단위가 아닌 경우 에러 처리 한다. (e.g. '1200')
    - [X] 문자로 입력한 경우 에러 처리 한다. (e.g. '만원')

- 당첨 번호
    - [X] 빈값으로 입력한 경우 에러 처리 한다. ('')
    - [X] 숫자 6개를 입력하지 않은 경우 에러 처리 한다. (e.g. '1, 3, 5, 6, 7')
    - [X] 숫자가 1에서 45까지가 아닐 경우 에러 처리 한다. (e.g. '1, 3, 4, 5, 46, 7')
    - [X] 6개 숫자 모두 컴마로 구분하지 않은 경우 에러 처리 한다. (e.g. '1. 3, 4, 5, 46, 7')
    - [X] 숫자가 중복되는 경우 에러 처리 한다. (e.g. '1, 1, 2, 3, 4, 5')

- 보너스 번호 
    - [ ] 빈값으로 입력한 경우 에러 처리 한다. ('')
    - [ ] 숫자가 아닌 경우 에러 처리 한다. ('칠')
    - [ ] 1부터 45까지의 숫자가 아닌 경우 에러 처리 한다. ('0'), ('100')
