...
### 2. 당첨 번호를 입력받는다. 번호는 쉼표기준으로 구분한다.
* 중복되지 않는 6개의 숫자를 입력받아야 한다. 
* 보너스 번호도 중복되지 않아야 한다.(이건 내가 정한 요구사항)
---
* 문자열을 입력받고
* 입력받은 숫자의 나열을 저장해둔다.
### 3. 보너스 번호를 입력받는다.
...
* 로또 한개의 가격은 1000원이고, 받은 돈만큼 발행해야한다.
* 이떄, 로또 숫자 범위는 1~45까지임
* 또한 하나의 로또에는 중복되지 않는 6개의 숫자를 뽑아야 한다.
---
* 몇개를 구매했는지 출력한다
* 발행한 로또들을 전부 출력한다. (로또번호는 오름차순)

### 5. 당첨 내역을 출력한다.
* 3개일치 ~ 6개 일치까지 받는 가격과 그것에 해당하는 로또가 몇개인지 출력한다.
...
### 라이브러리 요구사항
- [ ] MissionUtils.Random.pickUniqueNumbersInRange(1, 45, 6);와 Console.readLineAsync()와 Console.print()를 활용한다
...
# 🤔 개발하면서 신경쓴 부분
### 공통피드백 체크리스트
- [ ] README에서 프로젝트에 대한 설명과 주요기능이 무엇인지 소개했는가?
- [ ] 기능목록에 클래스설계, 메서드 설계같은 상세한 내용은 포함하지 않도록 했나?
- [ ] 기능목록을 작성할때 예외사항도 같이 정리했나?
- [ ] 기능목록을 작성할때 지속적으로 업데이트 했는가?
- [ ] 문자열이나 숫자값을 상수로 정의했는가?
- [ ] 클래스는 필드, 생성자, 메서드 순으로 작성했는가?
- [ ] 한 메서드가 한가지 기능만 담당하게 했는가?
- [ ] 작은 단위의 테스트부터 만들었는가?
- [ ] 클래스 이외에도 다른 객체 생성 방식을 익혔는가?
