...
## 함수 쪼개기
1. 안내 메시지를 출력하고 구입금액을 입력받는 함수
2. 구입 금액을 검증하고, 다음 단계로 넘어가거나 [ERROR]를 반환하는 함수
3. 구입 금액을 1000으로 나누어, 구매한 로또 개수를 계산하는 함수
4. 구매한 로또 개수만큼, 6개 랜덤 숫자로 이루어진 로또 목록을 생성하는 함수
5. 3번과 4번을 아래와 같은 형태로 출력하는 함수
```
8개를 구매했습니다.
[8, 21, 23, 41, 42, 43] 
[3, 5, 11, 16, 32, 38] 
...
```
6. 당첨번호를 입력받는 함수
7. 당첨번호를 검증하고, 다음 단계로 넘어가거나 [ERROR]를 반환하는 함수
8. 보너스번호를 입력받는 함수
9. 보너스번호를 검증하고, 다음 단계로 넘어가거나 [ERROR]를 반환하는 함수
10. 당첨번호와 보너스 번호를 구매한 로또들과 비교하여, 숫자가 n개 일치하는 로또의 개수를 계산하는 함수
11. 10번에서 계산된 당첨통계를 아래와 같이 출력하는 함수
```
당첨 통계
---
3개 일치 (5,000원) - 1개
