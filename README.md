...
- ❗️ 요구사항
  ...
  - 로또 1장의 가격은 1000원이다.
  - 사용자가 잘못된 값을 입력한 경우 `throw`문으로 예외를 발생시키고, `[ERROR]`로 시작하는 에러메시지를 출력한 후, 입력을 다시 받는다.
...
- 입력이 빈 값일 때 예외처리
- 입력이 숫자가 아닐 때 예외처리
- 구매 금액이 0이거나 음수일 때 예외처리
- 1회 구매한도 100000원 초과 시 예외처리
- 구매 금액이 1000으로 안나눠 떨어질 때 예외처리
- 당첨 번호 입력이 6자리 초과 시 예외처리
- 당첨 번호 입력 시 ,(쉼표)로 구분하지 않을 시 예외처리
- 당첨 번호가 1 ~ 45 사이 숫자가 아닐 시 예외처리
- 당첨 번호가 서로 중복일 시 예외처리
- 보너스 번호가 당첨 번호와 중복일 시 예외처리
