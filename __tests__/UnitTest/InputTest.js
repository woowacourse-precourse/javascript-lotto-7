...
describe('로또 번호 입력값 테스트', () => {
  ...
  test('로또 번호가 중복될 경우 예외가 발생한다.', async () => {
    ...
    await expect(input.requestLottoNumber()).rejects.toThrow(
      ERROR.DUPLICATED_NUMBER
    );
  });
...
describe('보너스 번호 입력값 테스트', () => {
  ...
  test('보너스 번호를 입력하지 않을 경우 예외가 발생한다.', async () => {
    mockQuestions('');
    await expect(input.requestBonusNumber(lottoNum)).rejects.toThrow(
      ERROR.BLANK
    );
  });
