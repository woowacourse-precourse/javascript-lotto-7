...
import { BOARD } from "../src/constant/boardMessage.js";
...
describe('Calculator 클래스 테스트', () => {
    ...
    describe('로또 게임 결과 계산', () => {
        test('1등 당첨 결과를 정확히 계산해야 한다', () => {
            ...
            expect(results.statistics.first.prize).toBe(BOARD.FIRST_WINNING_CASH);
        ...
        test('2등 당첨 결과를 정확히 계산해야 한다', () => {
            ...
            expect(results.statistics.second.prize).toBe(BOARD.SECOND_WINNING_CASH);
        });
    ...
    describe('수익률 관련 처리', () => {
        test('수익률을 정확히 계산해야 한다', () => {
            ...
            const expectedProfitRate = (BOARD.FIRST_WINNING_CASH / 1000) * 100;
            expect(results.profitRate).toBe(expectedProfitRate);
        });

        test('당첨금이 없을 때 수익률이 0이어야 한다', () => {
            const lottos = [new Lotto([7, 8, 9, 10, 11, 12])];
            const results = calculator.calculateGameResults(lottos, comparison);
