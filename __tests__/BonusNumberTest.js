...
describe("보너스 번호 테스트", () => {
  ...
  test.each([["3", [1, 2, 3, 4, 5, 6]]])(
    "보너스 번호와 로또 번호와 일치할 경우",
    async (input_number, input_lotto) => {
      expect(() => {
        NumberValidate.validateBonusDup(input_number, input_lotto);
      }).toThrow("[ERROR]");
    }
  );
