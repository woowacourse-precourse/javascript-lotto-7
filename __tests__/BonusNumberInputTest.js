import BonusNumberInput from '../src/Views/BonusNumberInput';
...
describe('보너스 번호 입력 Validate 테스트', () => {
  ...
  test('보너스 번호를 입력하지 않으면 다시 입력을 받아야 합니다.', () => {
    expect(BonusNumberInput.validate('', basicNumbers)).toEqual(false);
  });

  test('보너스 번호로 숫자를 입력하지 않으면 다시 입력을 받아야 합니다.', () => {
    expect(BonusNumberInput.validate('MinSungJe', basicNumbers)).toEqual(false);
  });

  test('보너스 번호로 1보다 작은 수를 입력하면 다시 입력을 받아야 합니다.', () => {
    expect(BonusNumberInput.validate('0', basicNumbers)).toEqual(false);
  });

  test('보너스 번호로 45보다 큰 수를 입력하면 다시 입력을 받아야 합니다.', () => {
    expect(BonusNumberInput.validate('800', basicNumbers)).toEqual(false);
  });

  test('보너스 번호가 당첨 번호와 중복되면 다시 입력을 받아야 합니다.', () => {
    expect(BonusNumberInput.validate('1', basicNumbers)).toEqual(false);
  });
