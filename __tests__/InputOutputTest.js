...
describe('입력문에 대한 테스트', () => {
  ...
  test('당첨 번호는 6개여야 한다.', () => {
    expect(() => {
      validation.winningNumber.isWrongLength(['1,2,3,4,5']).toThrow();
    });
  });
  ...
  test.each([['1,2,0,4,5,6'], ['1,46,34,12,42,40']])(
    '당첨번호의 범위는 1~45이다',
    (numberArr) => {
  ...
  test.each([['1,2,s,t,5,6'], ['@,2,3,6,TE,7'], [',,@,,#%,22']])(
    '당첨번호는 숫자여야 합니다.',
    (numberArr) => {
  ...
  test.each([['1,2.5,3,4,5,6'], ['1.2,1.1,2,5,6.6,7.9']])(
    '당첨번호는 정수여야 합니다.',
    (numberArr) => {
  ...
  test('당첨 번호는 중복되면 안 됩니다.', () => {
    expect(() => {
      validation.winningNumber.isNotDuplicate(['1,2,2,3,4,5']).toThrow();
    });
  });
});
