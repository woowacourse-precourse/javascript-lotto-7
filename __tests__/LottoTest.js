...
describe('로또 클래스 테스트', () => {
  const testCases = [
    {
      description:
        '로또 번호 6개가 1부터 45 사이의 중복되지 않는 경우 통과한다.',
    ...
    },
    {
      ...
      input: [1, 2, 3, 4, 5, 5],
      expectedError: ERROR_MESSAGES.invalid_duplicate_number,
    },
    {
      description: '로또 번호의 개수가 6개 초과할 경우 예외가 발생한다.',
      input: [1, 2, 3, 4, 5, 6, 7],
      expectedError: ERROR_MESSAGES.invalid_number_count,
    },
    {
    ...
    {
      description:
        '로또 번호가 1부터 45 사이의 범위를 벗어난 경우 예외가 발생한다.',
      input: [1, 2, 3, 4, 46],
      expectedError: ERROR_MESSAGES.invalid_number_range,
    },
  ];
  ...
  test.each(testCases)('$description', ({ input, expectedError }) => {
    ...
    expect(() => new Lotto(input).not.toThrow(expectedError));
  });
});
