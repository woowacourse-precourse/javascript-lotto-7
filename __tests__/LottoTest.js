...
import {ERROR_CODE} from "../src/constants/message.js";
...
describe("로또 클래스 테스트", () => {
    ...
    test("로또 번호의 개수가 6개가 넘어가면 예외가 발생한다.", () => {
        ...
        }).toThrow(ERROR_CODE.LOTTO_NOT_SIX);
    });

    test("로또 번호에 중복된 숫자가 있으면 예외가 발생한다.", () => {
        expect(() => {
            new Lotto([1, 2, 3, 4, 5, 5]);
        }).toThrow(ERROR_CODE.LOTTO_OVERLAP);
    });

    test("로또 번호에 1~45 이외의 숫자가 포함되어 있으면 예외가 발생한다.", () => {
        expect(() => {
            new Lotto([0, 2, 3, 4, 5, 6]);
        }).toThrow(ERROR_CODE.LOTTO_INVALID);
