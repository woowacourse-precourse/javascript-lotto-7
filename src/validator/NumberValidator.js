import { NUMBER_INPUT_ERROR_MESSAGE } from "../Constants.js";

///당첨번호 유효성
export function CheckWinningNumberInput() {
  if (!/^[0-9]+(,\s*[0-9]+)*$/.test(winningNumber)) {
    throw new Error(NUMBER_INPUT_ERROR_MESSAGE.ONLY_NUMBER_COMMA);
  }
}

export function isWinningNumberLength() {
...
// 보너스 번호 유효성 검사
export function isDuplicateBonusNumber() {
