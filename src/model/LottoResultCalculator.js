...
class LottoResultCalculator {
  compareWithWinningNumbers(randomNumbers, winningNumbers) {
    const remainingNumbers = randomNumbers
      .map((array) => array.filter((num) => !winningNumbers.includes(num)))
      .filter((array) => array.length > 0 && array.length < 6);

    const firstPlaceWinning = randomNumbers
      .map((array) => array.filter((num) => winningNumbers.includes(num)))
  ...
  compareWithBonusNumber(initialRemainingNumbers, bonusNumber) {
    const finalRemainingNumbers = initialRemainingNumbers.map((array) => array.filter((num) => num !== bonusNumber));
    return finalRemainingNumbers;
  }

  calculateWinningRank(randomNumbers, winningNumbers, bonusNumber) {
    const { firstPlaceWinning } = this.compareWithWinningNumbers(
      randomNumbers,
      winningNumbers,
    );
    const initialRemainingNumbers = this.compareWithWinningNumbers(
      randomNumbers,
      winningNumbers,
    ).remainingNumbers;
    const finalRemainingNumbers = this.compareWithBonusNumber(
      initialRemainingNumbers,
      bonusNumber,
    );

    const firstPlaceWinningCount = firstPlaceWinning.map(
      (array) => array.length,
    );
  ...
  calculateProfitRate(amount, rankCounts) {
    const totalPrize = Object.entries(rankCounts).reduce(
      (total, [key, count]) => total + count * LOTTO_PRIZE[key.toUpperCase()],
      0,
    ); // 일일이 호출해서 LOTTO_PRIZE와 더함 => chat GPT : Object.entries / toUpperCase를 이용해 value 매칭
    const profitRate = Number(
      ((totalPrize / amount) * 100).toFixed(1),
    ).toLocaleString();
    return profitRate;
  }
}
