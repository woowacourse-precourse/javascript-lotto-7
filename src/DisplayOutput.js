...
class DisplayOutput {
    ...
    displayLottoResult (lottoResult){
        ...
        */
        ...
        for (let i = 0; i < 3; i++) {
            MissionUtils.Console.print(`${i + 3}개 일치 (${LOTTO.PRIZE[prizeKeys[i]]}원) - ${lottoResult[0][i + 2]}개`);
        }
    
        // 보너스 당첨자 발표 및 6개 일치
        MissionUtils.Console.print(`5개 일치, 보너스 볼 일치 (${LOTTO.PRIZE.PRIZE_FIVE_NUMBER_BONUS_MATCHES}원) - ${lottoResult[1][0]}개`);
        MissionUtils.Console.print(`6개 일치 (${LOTTO.PRIZE.PRIZE_SIX_NUMBER_MATCHES}원) - ${lottoResult[0][5]}개`);
