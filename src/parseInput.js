...
export class ParseInput {
  ...
  parseMoney(inputString) {
    ...
    if (!Number.isInteger(floatvalue)) throw new Error(ERROR_MESSAGE_MONEY_INPUT.nonInteger);
    this.#money = value;
    return this.#money;
  }

  #processJackpot(jackpotArray) {
    this.#jackpot = jackpotArray
      .map((value) => parseInt(value))
      .reduce((previous, current) => {
        if (previous.includes(current)) throw new Error(ERROR_MESSAGE_JACKPOT_INPUT.duplicated);
  ...
  parseBonus(inputString) {
    ...
    if (!Number.isInteger(floatvalue)) throw new Error(ERROR_MESSAGE_BONUS_INPUT.nonInteger);
    this.#bonus = value;
    return this.#bonus;
  }
}
