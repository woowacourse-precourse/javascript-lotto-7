...
class App {
  ...
  async start() {
    ...
    const input = await Console.readLineAsync("구입금액: ");
  ...
  async getWinningNumbers() {
    ...
    const input = await Console.readLineAsync("당첨 번호: ");
    ...
    if (this.validateWinningNumbers(winningNumbers)) {
      ...
      await this.getWinningNumbers(); // 잘못된 입력이므로 다시 입력 받기
    }
  }
  ...
  async getBonusNumber() {
    ...
    const input = await Console.readLineAsync("보너스 번호: ");
    ...
    if (this.validateBonusNumber(bonusNumber)) {
      ...
      await this.getBonusNumber(); // 잘못된 입력이므로 다시 입력 받기
    }
  }
  ...
  printResults(results, prizeTable) {
    ...
    const profitRate = this.calculateYield(totalPrize);
  ...
  calculateYield(totalPrize) {
    const yieldRate = (totalPrize / this.purchaseAmount) * 100;
